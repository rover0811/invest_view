...
| `KISRawMessageParser` | `0|TR_ID|건수|payload`를 레코드 리스트로 분해 |
...
1. 시스템 시작 시 REST access token을 확보한다.
2. WebSocket approval key를 발급받는다.
3. `KISConnectionManager`가 단일 세션을 연다.
4. `KISSubscriptionPool`의 현재 union을 기준으로 종목 등록을 수행한다.
5. 수신된 raw message는 parser -> tick parser -> Kafka producer 순서로 처리된다.
...
- `RawKISResponse` / `RawKISTickRecord`: KIS 문서 필드명에 최대한 가깝게 유지
...
- approval key와 access token 만료 시점을 어떻게 선제 갱신할 것인가?
