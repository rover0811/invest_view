...
| Publishing | IngestionService가 AsyncIterator를 소비하여 Kafka `stock-ticks`에 발행 |
...
| `KISConnectionManager` | 1세션 제약 하에서 연결 lifecycle 관리. reconnect + resubscribe 총괄. AsyncIterator를 통한 tick 공급 | `src/kis_ingestion/connection_manager.py` |
...
- output sink 추상화(TickSink)를 두지 않는다. Kafka 단계는 `StockTickProducer`를 직접 도입한다. `ConnectionManager`는 AsyncIterator로서 tick을 yield하고, `IngestionService`가 이를 소비하여 producer로 발행한다.
...
7. `IngestionService`가 `KISConnectionManager`의 AsyncIterator를 소비하며, 수신된 tick을 `StockTickProducer`를 통해 Kafka로 발행한다.
...
4. `KISConnectionManager`가 `(ParsedTick, session_id, sequence)` 튜플을 yield한다.
5. `IngestionService`가 이를 소비하여 `StockTickProducer`를 호출한다.
6. `StockTickProducer`는 local `stock-ticks.avsc` 기준으로 Avro 직렬화 후, key=`symbol`, headers=`session_id`/`sequence`로 `stock-ticks`를 발행한다.
7. downstream(Flink / custom persistence consumer)은 여기서부터만 KIS 세부 포맷을 몰라도 된다.
...
- `service.py`에서 `connection_manager`의 AsyncIterator를 소비하며 producer publish 수행
