"""Kafka consumer for stock-patterns topic with Confluent Schema Registry support.
...
class PatternConsumer:
    ...
    async def start(self) -> None:
        ...
        self._poll_task = asyncio.create_task(self._run_poll_loop(), name="pattern-consumer-poll")
        self._dispatch_task = asyncio.create_task(self._run_dispatch_loop(), name="pattern-consumer-dispatch")
        self._poll_task.add_done_callback(self._on_task_done)
        self._dispatch_task.add_done_callback(self._on_task_done)
        logger.info("pattern consumer started topic=%s group=%s", self._topic, self._consumer.list_topics(timeout=2.0))
    ...
    def _on_task_done(self, task: asyncio.Task[None]) -> None:
        ...
        logger.error("pattern consumer task failed: %s", task.get_name(), exc_info=(type(error), error, error.__traceback__))
        self._dead.set()
