...
async def _snapshot_impl(session_factory: _SessionFactory, ticker: str) -> dict[str, object]:
    async with session_factory() as session:
        rows = await fetch_snapshot(session, [ticker])
...
async def get_symbol_snapshot(tool_context: ToolContext) -> dict[str, object]:
    ...
    return await _snapshot_impl(session_factory, ticker)
