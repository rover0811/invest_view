...
- 현재 planning phase 기준으로는 `stock-ticks`를 raw bridge에 가깝게 둘지, `ParsedTick` 수준의 normalized contract로 둘지 **아직 최종 확정하지 않았다**.
- 다만 어떤 방향을 택하든, handoff contract는 문서에서 **정확한 필드 스키마**로 고정해야 한다.
...
- 지금 단계에서는 `stock-ticks`를 raw bridge로 둘지 normalized contract로 둘지 **명시적으로 결정**하는 것이 중요하다.
- 만약 raw bridge로 간다면, 이 문서는 DB ERD보다 `stock-ticks`의 **정확한 raw handoff schema**를 우선 기재해야 한다.
- 만약 normalized contract로 간다면, 그때는 freeze 문서의 event schema와 맞춘 projection 근거를 더 명확히 써야 한다.
- order book / quote depth 확장을 시작하면 `ASKP1`, `BIDP1`, 잔량 계열은 별도 extended tick schema나 별도 topic 후보가 된다.

### Candidate normalized `ParsedTick` contract

만약 `stock-ticks`를 normalized contract로 둘 경우, `ParsedTick`은 최소 아래 필드를 포함할 수 있다.
...
- `stock-ticks`를 raw bridge contract로 둘지, normalized contract로 둘지 최종적으로 어떤 방향을 채택할 것인가?
- raw bridge를 택한다면 `stock-ticks`에 어떤 필드를 어떤 이름/타입으로 정확히 실을 것인가?
