...
### `stock-ticks` v1 raw value contract

현재 v1 `stock-ticks`는 KIS WebSocket 체결 모델의 semantic field set을 **raw-oriented handoff schema**로 사용한다.
...
| 체결강도 / 체결건수 | `SELN_CNTG_CSNU`, `SHNU_CNTG_CSNU`, `NTBY_CNTG_CSNU`, `CTTR`, `SELN_CNTG_SMTN`, `SHNU_CNTG_SMTN`, `CCLD_DVSN` (`CNTG_CLS_CODE` alias), `SHNU_RATE`, `PRDY_VOL_VRSS_ACML_VOL_RATE` |
...
- `CCLD_DVSN` / `CNTG_CLS_CODE` alias 차이를 v1 계약에서 어떤 canonical field name으로 정리할 것인가?
