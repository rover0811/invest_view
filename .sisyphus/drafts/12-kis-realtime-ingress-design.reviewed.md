...
`[[11-design-freeze-discussion-pack]]`의 ERD는 프로젝트 전체 기준이고, 아래 다이어그램은 그중 **KIS scope가 직접 책임지는 부분만 잘라낸 conceptual slice**다.

### Clarification

- 이 ERD는 **현재 구현 상태를 반영한 schema dump가 아니라 설계용 logical model**이다.
- `bronze.tick_history`, `integration.realtime_subscriptions`는 상위 freeze 문서의 schema proposal에 근거가 있다.
- 반면 `integration.kis_connection_sessions`는 KIS 상세 설계에서 추가된 **KIS-scope proposal**이며, 현재 기준으로 migration/DDL 근거는 없다.
- 따라서 planning PR에서는 이 ERD를 확정 schema라기보다 **구현 후보 구조**로 읽는 편이 맞다.
...
- 다만 `.env`에서 읽은 값 자체를 별도 settings component로 승격하는 것보다, auth bootstrap 단계에서 필요한 입력만 token / approval manager에 주입하는 쪽이 현재 repo 규모에는 더 간결하다.
...
- `stock-ticks`는 KIS raw field index를 그대로 노출하는 topic이 아니라, downstream이 안정적으로 소비할 수 있는 **domain contract**여야 한다.
- 따라서 KIS 전용 인덱스/필드 배치를 그대로 외부 계약으로 노출하기보다, freeze 문서의 event schema에서 이미 합의한 핵심 필드 중심 projection으로 정리하는 편이 낫다.
- 이 방식이면 downstream(Flink / persistence consumer / serving)이 KIS payload layout 변화와 직접 결합되지 않는다.
